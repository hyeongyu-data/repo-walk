...
- 각 스레드를 **지적한 내용 → 배경 개념(왜 그게 문제/궁금인지) → 저자가 어떻게
  반영·답변했고 타당한지** 순으로. 팀원 문답도 학습 자료로 포함.

**(4) 직접 코드리뷰** — 당신이 **직접** diff를 정독해 기존 리뷰가 **놓친**
개선점·위험·실수 가능성을 찾습니다(기존 리뷰가 이미 잡은 건 중복 금지):
- Terraform/코드 스타일·보안·설계·비용·유지보수·문서 정합성 등 관점에서.
- 각 발견마다 **파일·라인**, 무엇이 문제인지, 구체적 개선안을 심각도 순으로.
- **기존 리뷰의 주장도 비판적으로 검증**하세요 — 리뷰나 코드 주석이 틀릴 수
  있습니다. 사실 오류를 발견하면 근거와 함께 바로잡습니다.
- 이건 당신의 추론이므로 **신뢰도(높음/중간/낮음)를 표기**하고, diff에서 근거를
  못 찾으면 지어내지 말고 "해당 없음".

**(5) 학습 포인트** — 이 PR에서 배울 것을 **범주로 나눠** 확장합니다. diff·리뷰를
읽은 당신의 **추론이자 교육용 참고**이며 확정이 아닙니다(그렇게 명시):
- **개념**: 이 PR로 배울 핵심 개념.
- **모범 사례**: 따라 하면 좋은 것.
