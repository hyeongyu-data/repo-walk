...
(기본 1 — 한 번에 PR 하나). **각 단위마다** 다음 4부분을 순서대로 냅니다:

**(1) 헤더:** `[type #id · date] title`

**(2) 해설(이게 핵심 가치입니다)** — 2~4문장:
- PR 중심: 이 PR이 무엇을 바꿨는지, 왜(본문이 `#N` / `closes #N`을 참조하면
  연결된 이슈를 끌어옴), 앞 단위 위에 어떻게 쌓이는지.
- 시간순: 이 단계에서 무슨 일이 있었고 앞 단계와 어떻게 이어지는지.

**(3) 리뷰(있으면)** — PR에 달린 리뷰와 코드리뷰 코멘트는 "왜 이렇게
결정했나"의 근거이므로 해설에 녹입니다. 없으면 "리뷰 없음"으로 한 줄 처리:
- 리뷰 판정·요약: `gh pr view N -R O/R --json reviews --jq '.reviews[] | {author:.author.login, state:.state, body:.body}'`
- 인라인 코드리뷰 코멘트: `gh api repos/O/R/pulls/N/comments --jq '.[] | {path:.path, line:.line, author:.user.login, body:.body}'`
- 리뷰에서 지적된 내용과 그에 따른 수정이 있으면, "리뷰어가 X를 지적 → 이렇게
  반영" 식으로 압축해 보여줍니다.

**(4) 학습 포인트** — 이 PR에서 배울 것을 3항목으로. 이건 diff·리뷰를 읽은
당신의 **추론이자 교육용 참고**이며 확정이 아닙니다(그렇게 명시):
- 개선점: 더 낫게 할 수 있었던 부분(있으면).
- 흔히 하는 실수: 이런 종류의 변경에서 사람들이 자주 저지르는 실수.
- 주의할 점: 리뷰·롤백·부작용 관점에서 조심할 지점.
